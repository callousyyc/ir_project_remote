...
/// @see Panasonic A/C support heavily influenced by
...
uint64_t IRsend::encodePanasonic(const uint16_t manufacturer,
                                 const uint8_t device, const uint8_t subdevice,
...
bool IRrecv::decodePanasonic(decode_results *results, uint16_t offset,
  ...
  if (!matchGeneric(results->rawbuf + offset, &data, results->rawlen - offset,
                    nbits, kPanasonicHdrMark, kPanasonicHdrSpace,
                    kPanasonicBitMark, kPanasonicOneSpace, kPanasonicBitMark,
                    kPanasonicZeroSpace, kPanasonicBitMark, kPanasonicEndGap,
                    true))
    return false;
  // Compliance
...
uint16_t IRPanasonicAc::_getTime(const uint8_t ptr[]) {
  uint16_t result = (GETBITS8(ptr[1], kLowNibble, kPanasonicAcTimeOverflowSize)
...
uint8_t IRPanasonicAc::convertFan(const stdAc::fanspeed_t speed) {
  ...
  case stdAc::fanspeed_t::kLow:
    return kPanasonicAcFanLow;
  case stdAc::fanspeed_t::kMedium:
    return kPanasonicAcFanMed;
  case stdAc::fanspeed_t::kHigh:
    return kPanasonicAcFanHigh;
...
String IRPanasonicAc::toString(void) {
  ...
  result += addSwingVToString(getSwingVertical(), kPanasonicAcSwingVAuto,
                              kPanasonicAcSwingVHighest, kPanasonicAcSwingVHigh,
                              kPanasonicAcSwingVAuto, // Upper Middle is unused
                              kPanasonicAcSwingVMiddle,
                              kPanasonicAcSwingVAuto, // Lower Middle is unused
                              kPanasonicAcSwingVLow, kPanasonicAcSwingVLowest,
                              // Below are unused.
                              kPanasonicAcSwingVAuto, kPanasonicAcSwingVAuto,
                              kPanasonicAcSwingVAuto, kPanasonicAcSwingVAuto);
  switch (getModel()) {
  case kPanasonicJke:
  case kPanasonicCkp:
  ...
  default:
    result += addSwingHToString(
        getSwingHorizontal(), kPanasonicAcSwingHAuto,
        kPanasonicAcSwingHFullLeft, kPanasonicAcSwingHLeft,
        kPanasonicAcSwingHMiddle, kPanasonicAcSwingHRight,
        kPanasonicAcSwingHFullRight,
        // Below are unused.
        kPanasonicAcSwingHAuto, kPanasonicAcSwingHAuto, kPanasonicAcSwingHAuto,
        kPanasonicAcSwingHAuto, kPanasonicAcSwingHAuto);
  }
...
bool IRrecv::decodePanasonicAC(decode_results *results, uint16_t offset,
  ...
  uint16_t used;
  used = matchGeneric(
      results->rawbuf + offset, results->state, results->rawlen - offset,
      kPanasonicAcSection1Length * 8, kPanasonicHdrMark, kPanasonicHdrSpace,
      kPanasonicBitMark, kPanasonicOneSpace, kPanasonicBitMark,
      kPanasonicZeroSpace, kPanasonicBitMark, kPanasonicAcSectionGap, false,
  ...
  if (!matchGeneric(
          results->rawbuf + offset, results->state + kPanasonicAcSection1Length,
          results->rawlen - offset, nbits - kPanasonicAcSection1Length * 8,
          kPanasonicHdrMark, kPanasonicHdrSpace, kPanasonicBitMark,
          kPanasonicOneSpace, kPanasonicBitMark, kPanasonicZeroSpace,
  ...
  if (strict) {
    ...
    if (results->state[0] != 0x02 || results->state[1] != 0x20 ||
      ...
      return false;
    if (!IRPanasonicAc::validChecksum(results->state, nbits / 8))
      return false;
  }
  ...
  for (uint16_t r = 0; r <= repeat; r++) {
    for (uint8_t section = 0; section < sections; section++) {
      ...
      sendGeneric(kPanasonicAc32HdrMark, kPanasonicAc32HdrSpace, // Header
                  ...
                  kPanasonicAc32BitMark, kPanasonicAc32ZeroSpace, 0,
                  0, // No Footer
...
bool IRrecv::decodePanasonicAC32(decode_results *results, uint16_t offset,
                                 const uint16_t nbits, const bool strict) {
  if (strict &&
      (nbits != kPanasonicAc32Bits && nbits != kPanasonicAc32Bits / 2))
  ...
  const uint16_t min_length =
      is_long ? kPanasonicAc32Sections * kPanasonicAc32BlocksPerSection *
  ...
  for (uint16_t block = 0; block < sections * blocks_per_section; block++) {
    ...
    uint16_t used = matchGeneric(
        results->rawbuf + offset, &section_data, results->rawlen - offset,
        bits_per_block * 2, kPanasonicAc32HdrMark, kPanasonicAc32HdrSpace,
        kPanasonicAc32BitMark, kPanasonicAc32OneSpace, kPanasonicAc32BitMark,
        kPanasonicAc32ZeroSpace, 0, 0, // No Footer
    ...
    if (block % blocks_per_section == 0) {
      ...
      if ((block + 1) % blocks_per_section == 0) {
        uint64_t junk;
        used = matchGeneric(
            results->rawbuf + offset, &junk, results->rawlen - offset, 0,
            // Header
            kPanasonicAc32HdrMark, kPanasonicAc32HdrSpace,
            // No Data
            0, 0, 0, 0,
            // Footer
            kPanasonicAc32BitMark, kPanasonicAc32SectionGap, true);
        if (!used)
          return false;
        offset += used;
      }
...
uint8_t IRPanasonicAc32::convertMode(const stdAc::opmode_t mode) {
  switch (mode) {
  case stdAc::opmode_t::kCool:
    return kPanasonicAc32Cool;
  case stdAc::opmode_t::kHeat:
    return kPanasonicAc32Heat;
  case stdAc::opmode_t::kDry:
    return kPanasonicAc32Dry;
  case stdAc::opmode_t::kFan:
    return kPanasonicAc32Fan;
  default:
    return kPanasonicAc32Auto;
  }
}
...
uint8_t IRPanasonicAc32::convertFan(const stdAc::fanspeed_t speed) {
  switch (speed) {
  case stdAc::fanspeed_t::kMin:
    return kPanasonicAc32FanMin;
  case stdAc::fanspeed_t::kLow:
    return kPanasonicAc32FanLow;
  case stdAc::fanspeed_t::kMedium:
    return kPanasonicAc32FanMed;
  case stdAc::fanspeed_t::kHigh:
    return kPanasonicAc32FanHigh;
  case stdAc::fanspeed_t::kMax:
    return kPanasonicAc32FanMax;
  default:
    return kPanasonicAc32FanAuto;
  }
}
...
uint8_t IRPanasonicAc32::convertSwingV(const stdAc::swingv_t position) {
  ...
  case stdAc::swingv_t::kLowest:
    return (uint8_t)position;
  default:
    return kPanasonicAc32SwingVAuto;
  }
}
...
String IRPanasonicAc32::toString(void) const {
  ...
  result +=
      addModeToString(_.Mode, kPanasonicAc32Auto, kPanasonicAc32Cool,
                      kPanasonicAc32Heat, kPanasonicAc32Dry, kPanasonicAc32Fan);
  result += addTempToString(getTemp());
  ...
  result +=
      addSwingVToString(getSwingVertical(), kPanasonicAc32SwingVAuto,
                        kPanasonicAcSwingVHighest, kPanasonicAcSwingVHigh,
                        ...
                        kPanasonicAcSwingVLow, kPanasonicAcSwingVLowest,
                        // Below are unused.
                        kPanasonicAc32SwingVAuto, kPanasonicAc32SwingVAuto,
                        kPanasonicAc32SwingVAuto, kPanasonicAc32SwingVAuto);
  return result;
}
#endif
...
stdAc::state_t IRPanasonicAc32::toCommon(const stdAc::state_t *prev) const {
  ...
  result.swingv = toCommonSwingV(getSwingVertical());
  result.swingh =
      getSwingHorizontal() ? stdAc::swingh_t::kAuto : stdAc::swingh_t::kOff;
  // Not supported.
  result.quiet = false;
  result.turbo = false;
  result.filter = false;
