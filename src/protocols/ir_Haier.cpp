...
uint8_t IRHaierAC::getCommand(void) const { return _.Command; }
...
uint8_t IRHaierAC::getMode(void) const { return _.Mode; }
...
uint8_t IRHaierAC::getTemp(void) const { return _.Temp + kHaierAcMinTemp; }
...
bool IRHaierAC::getHealth(void) const { return _.Health; }
...
/// @return true, the setting is on. false, the setting is off.
bool IRHaierAC::getSleep(void) const { return _.Sleep; }
...
void IRHaierAC::setCurrTime(const uint16_t nr_mins) { SETTIME(Curr, nr_mins); }

/// Get the Vertical Swing position setting of the A/C.
/// @return The native vertical swing mode.
uint8_t IRHaierAC::getSwingV(void) const { return _.SwingV; }
...
uint8_t IRHaierAC::convertSwingV(const stdAc::swingv_t position) {
  ...
  case stdAc::swingv_t::kMiddle:
    return kHaierAcSwingVUp;
  case stdAc::swingv_t::kLow:
  case stdAc::swingv_t::kLowest:
    return kHaierAcSwingVDown;
  case stdAc::swingv_t::kOff:
    return kHaierAcSwingVOff;
  default:
    return kHaierAcSwingVChg;
  }
}
...
uint8_t IRHaierAC176::getButton(void) const { return _.Button; }
...
uint8_t IRHaierAC176::getSwing(void) const { return IRHaierAC176::getSwingV(); }
...
void IRHaierAC176::setSwingH(uint8_t pos) {
  ...
  case kHaierAcYrw02SwingHAuto:
    _.Button = kHaierAcYrw02ButtonSwingH;
    break;
  default:
    return; // Unexpected value so don't do anything.
  }
...
void IRHaierAC176::setTimerMode(const uint8_t mode) {
  _.TimerMode =
      (mode > kHaierAcYrw02OffThenOnTimer) ? kHaierAcYrw02NoTimers : mode;
  switch (_.TimerMode) {
...
uint8_t IRHaierAC176::convertSwingV(const stdAc::swingv_t position) {
  ...
  case stdAc::swingv_t::kHighest:
  case stdAc::swingv_t::kHigh:
    return kHaierAcYrw02SwingVTop;
  case stdAc::swingv_t::kMiddle:
    return kHaierAcYrw02SwingVMiddle;
  case stdAc::swingv_t::kLow:
    return kHaierAcYrw02SwingVDown;
  case stdAc::swingv_t::kLowest:
    return kHaierAcYrw02SwingVBottom;
  case stdAc::swingv_t::kOff:
    return kHaierAcYrw02SwingVOff;
  default:
    return kHaierAcYrw02SwingVAuto;
  }
}
...
uint8_t IRHaierAC176::convertSwingH(const stdAc::swingh_t position) {
  ...
  case stdAc::swingh_t::kMiddle:
    return kHaierAcYrw02SwingHMiddle;
  case stdAc::swingh_t::kLeftMax:
    return kHaierAcYrw02SwingHLeftMax;
  case stdAc::swingh_t::kLeft:
    return kHaierAcYrw02SwingHLeft;
  case stdAc::swingh_t::kRight:
    return kHaierAcYrw02SwingHRight;
  case stdAc::swingh_t::kRightMax:
    return kHaierAcYrw02SwingHRightMax;
  case stdAc::swingh_t::kAuto:
    return kHaierAcYrw02SwingHAuto;
  default:
    return kHaierAcYrw02SwingHMiddle;
  }
}
...
String IRHaierAC176::toString(void) const {
  ...
  result += ')';
  result +=
      addModeToString(_.Mode, kHaierAcYrw02Auto, kHaierAcYrw02Cool,
                      kHaierAcYrw02Heat, kHaierAcYrw02Dry, kHaierAcYrw02Fan);
  ...
  result += addSwingHToString(
      _.SwingH, kHaierAcYrw02SwingHAuto, kHaierAcYrw02SwingHLeftMax,
      kHaierAcYrw02SwingHLeft, kHaierAcYrw02SwingHMiddle,
      kHaierAcYrw02SwingHRight, kHaierAcYrw02SwingHRightMax,
      // Below are unused.
      kHaierAcYrw02SwingHMiddle, kHaierAcYrw02SwingHMiddle,
      kHaierAcYrw02SwingHMiddle, kHaierAcYrw02SwingHMiddle,
  ...
  result += ')';
  result += addLabeledString(
      (tmode != kHaierAcYrw02NoTimers && tmode != kHaierAcYrw02OffTimer)
          ? minsToString(getOnTimer())
          : kOffStr,
      kOnTimerStr);
  result += addLabeledString(
      (tmode != kHaierAcYrw02NoTimers && tmode != kHaierAcYrw02OnTimer)
          ? minsToString(getOffTimer())
          : kOffStr,
      kOffTimerStr);
...
IRHaierACYRW02::IRHaierACYRW02(const uint16_t pin, const bool inverted,
                               const bool use_modulation)
    : IRHaierAC176(pin, inverted, use_modulation) {
  stateReset();
}
...
bool IRrecv::decodeHaierAC(decode_results *results, uint16_t offset,
  ...
  // Pre-Header
  if (!matchMark(results->rawbuf[offset++], kHaierAcHdr))
    return false;
  if (!matchSpace(results->rawbuf[offset++], kHaierAcHdr))
    return false;

  // Match Header + Data + Footer
  if (!matchGeneric(results->rawbuf + offset, results->state,
                    results->rawlen - offset, nbits, kHaierAcHdr,
                    kHaierAcHdrGap, kHaierAcBitMark, kHaierAcOneSpace,
                    kHaierAcBitMark, kHaierAcZeroSpace, kHaierAcBitMark,
                    kHaierAcMinGap, true, _tolerance, kMarkExcess))
    return false;
  ...
  if (strict) {
    if (results->state[0] != kHaierAcPrefix)
      return false;
    if (!IRHaierAC::validChecksum(results->state, nbits / 8))
      return false;
  }
...
bool IRrecv::decodeHaierACYRW02(decode_results *results, uint16_t offset,
  ...
  if (!decodeHaierAC(results, offset, nbits, false))
    return false;
...
bool IRrecv::decodeHaierAC176(decode_results *results, uint16_t offset,
  ...
  if (strict) {
    if ((results->state[0] != kHaierAcYrw02ModelA) &&
      ...
      return false;
    if (!IRHaierAC176::validChecksum(results->state, nbits / 8))
      return false;
  }
